...
public interface CallDataRepository extends JpaRepository<CallData, String> {
    ...
    @Query("SELECT new com.example.webmorda_backend.model.DispositionCount(c.disposition,COUNT(*)) FROM CallData c WHERE c.calldate between ?1 AND ?2 GROUP BY c.disposition")
